...
- **Sync:** the recruit channel is checked first; then voice channels (`🔵 Команда A · #<id>`, overwrites replaced in full, fake ids never passed), the recruitment message (reposted if deleted), one announcement per status (`announcedStatus`), orphan cleanup after a terminal sync, finally `syncedVersion = v WHERE syncedVersion < v`. It never bumps `version`.
- **A recruit channel that does not resolve (2026-09-20, amended by the review the same day):** it is the one id `sync` cannot re-create (NOT NULL, and the match was created in it). `checkRecruitChannel` answering `NotFound` — deleted, left in the server the bot served before, or simply invisible to the bot (50001 in this guild too) — stops the sync before any Discord work and says so once per (match, version) in the log channel: «бот не видит канал набора — верни боту доступ», with «отмени матч в «/игры»» as the second option (nothing actionable for a terminal match). `syncedVersion` is deliberately **not** stamped: the match stays in `unsynced()`, the minute job keeps retrying, and access given back repairs the message by itself. A missing *permission* other than View is not `NotFound` and still fails loudly. A match nobody can see is cancelled by a person; `cancel` needs no Discord.
