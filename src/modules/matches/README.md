...
- **A recruit channel that no longer resolves (2026-09-20):** it is the one id `sync` cannot re-create (NOT NULL, and the match was created in it). `checkRecruitChannel` answering `NotFound` — deleted, or left in the server the bot served before — stops the sync before any Discord work, says so once per (match, version) in the log channel with «отмени матч в «/игры»» (nothing actionable for a terminal match), and records `syncedVersion` so the minute job stops retrying. A missing *permission* is not `NotFound` and still fails loudly. The match is cancelled by a person; `cancel` needs no Discord.
